...
.factory('PrintsContainerComponent', ['$state', '$controller', 'StoreConfig', 'AngularDropdownComponent', 'GeomService',
    ...
    function($state, $controller, StoreConfig, AngularDropdownComponent, GeomService,
        ...
        return React.createClass({
            ...
            onAfterRender: function() {
                ...
                if (layoutSize.gridX>0 && numImages>0) {
                    ...
                    for (var i = 0; i < layoutSize.gridX - 1; i++) {
                        ctx.beginPath();
                        ctx.moveTo(Math.floor(x), Math.floor(r.y));
                        ctx.lineTo(Math.floor(x), Math.floor(r.y + r.height));
                        ctx.stroke();
                        x += w;    
                    }
                    for (var i = 0; i < layoutSize.gridY - 1; i++) {
                        ctx.beginPath();
                        ctx.moveTo(Math.floor(r.x), Math.floor(y));
                        ctx.lineTo(Math.floor(r.x + r.width), Math.floor(y));
                        ctx.stroke();
                        y += h;    
                    }
                    if (!this.props.isSelected) {
                        ctx.strokeRect(r.x, r.y, r.width - 1, r.height - 1);
                    }
                    ctx.restore();
                } 
            ...
            setCurrentImage: function(currentSpread, animate, delay) {
                ...
                if (!layoutSize.templateSpread && !(layoutSize.gridX>0)) {
            ...
            getCurrentElement: function() {
                var spread = this.getCurrentSpread();
                if (spread) {
                    return _.findWhere(spread.elements, {type:'ImageElement'});
                }
...
//------------------------------------------------------
            ...
            componentDidUpdate: function() {
                ...
                if (this.selectCurrentImage) {
                    ...
                    var element = _.findWhere(this.renderer.spread.elements, {type:'ImageElement'});
                    if (this.mouseTarget && this.mouseTarget.type!=='BackgroundFrameElement' &&
                        this.renderer.spread.elements.indexOf(this.mouseTarget)>=0) {
                ...
                if (!this.isAnimating && this.doTransition) {
                    ...
                    var shouldCenterImages = !(this.props.container.layoutSize.gridX>0 || this.props.container.layoutSize.templateSpread);
...
//------------------------------------------------------
            ...
            onSelectionModifying: function(e, element) {
                ...
                if (isGrid) {
                    _.each(this.renderer.spread.elements, function(el, i) {
                        if (el.type==='ImageElement') {
                            _.extend(el, _.pick(element, 'imageX', 'imageY', 'imageWidth', 
                                'imageHeight', 'imageRotation', 'errors'));
                        }
                    });
                    this.renderer.render();
                }
            ...
            onSelectionModified: function() {
                ...
                var ctrl = this.props.layoutController,
                    ...
                    orientationChanged,
                    layoutSize = this.props.container.layoutSize,
                    forceUpdate = false;

                if (element && element.type==='ImageElement' && 
                    !layoutSize.templateSpread && !layoutSize.gridX>0) {
                ...
                _.each(this.renderer.spread.elements, function(el, i) {
					...
					//TODO: only for grid
                    if (layoutSize.gridX>0 && el.type==='ImageElement') {
                    ...
                    } else if (el.type==='TextElement' || el.type==='SpineTextElement') {
            ...
            onContainerSizeChange: function(event, id) {
                ...
                var isGrid = container.layoutSize.gridX>0;
                ...
                for (var i = 0; i < container.spreads.length; i++) {
                    ...
                    if (isGrid) {
                        var layoutSize = container.layoutSize;
                        var prevW = prevLayoutSize.width / prevLayoutSize.gridX,
                            prevH = prevLayoutSize.height / prevLayoutSize.gridY,
                            w = layoutSize.width / layoutSize.gridX,
                            h = layoutSize.height / layoutSize.gridY,
                            prevSizeType = this.getSizeType({width: prevW, height:prevH}),
                            sizeType = this.getSizeType({width: w, height:h}),
                            orientationChanged = prevSizeType!=sizeType;
                     
                        for (var xx = 0; xx < layoutSize.gridX; xx++) {
                            for (var yy = 0; yy < layoutSize.gridY; yy++) {
                                var idx = xx + (yy * layoutSize.gridX);
                                var el = spread.elements[idx];
                                if (!el) {
                                    el = _.clone(spread.elements[0]);
                                    delete el.id;
                                    delete el.version;
                                    spread.elements[idx] = el;
                                }
                                
                                if (orientationChanged) {
                                    el.width = w;
                                    el.height = h;
                                    new PACE.FillFrameAndCenterCommand(el).execute();
                                } else {
                        ...
                        spread.elements = spread.elements.slice(0, (layoutSize.gridX * layoutSize.gridY));
                    } else {
            ...
            renderAnim: function() {
                ...
                var width = Math.round((layoutSize.width + (layoutSize.bleedOutside * 2)) * scale) + 3;
                var height = Math.round((layoutSize.height + (layoutSize.bleedOutside * 2)) * scale) + 3;
                ...
                //console.log('renderAnim', isSelected)
                ...
                //var toolbarsVisible = viewMode!=='filmstrip';
                var liStyle = {};
                var margin = 15;
...
//------------------------------------------------------
            render: function() {
                if (!this.props.container) return (<li className="prints-container"></li>);
                ...
                if (layoutSize.gridX>0) {
                    deleteIconStyle.right = '7px';
                    deleteIconStyle.top = '10px';
                } else if (currentElement && this.renderer) {
                    var el = this.props.layoutController.selectedElements[0] || currentElement;
                    var bbox = new PACE.Element(el).getBoundingBox().toCanvasSpace(this.renderer.canvas);
                    deleteIconStyle.right = ( (width - (bbox.right + left)) + 7) + 'px';
                    deleteIconStyle.top = ((bbox.top - top) + 10) + 'px';
                }
               
                return (
                    <li className={liClasses} style={liStyle} ref="itemRoot">

                        <div className={contentClasses} style={contentStyle} ref="contentDiv"
                            onClick = {this.onContainerClick}
                            onDragOver = {this.onContainerDragOver}
                            onDragLeave = {this.onContainerDragLeave}
                            onDrop = {this.onContainerDrop}>

                            <div className="prints-container__box prints-container__box--primary">
                                {
                                    isGrid && numImages===0 &&
                                    <div className="prints-container__grid" style={gridStyle}>
                                    {
                                        _.times(layoutSize.gridX * layoutSize.gridY, function(i) {
                                            return (
                                                <div className="prints-container__grid-cell">
                                                    <div className="prints-container__placeholder">
                                                        {containerSize}
                                                        <div className="prints-container__placeholder-secondary-label">Drag image(s) here</div>
                                                    </div>
                                                </div>
                                            )}
                                        )
...

                            {
                                ...
                                <div className="prints-photo-copies" data-pace-tooltip="Copies of this photo">
                                    ...
                                    <input ref="imageQtyInput" className="prints-photo-copies__input" type="text"
                                        disabled={!this.props.editable}
                        ...
                        { 
                            ...
                            <img className="prints-container__image-remove"
...

                        {
                            ...
                            <img className="prints-container__remove"
