        ...
        return React.createClass({
            ...
            render: function() {
                ...
                return (
                    ...
                    <li id={this.props.id} style={liStyles} className={classes} draggable="true"
                        ...
                        <div className="filmstrip-item-inner">

                            {
                                this.props.showAdminInfo ? <span style={iccProfileStyle} className="admin-info">{iccProfile}</span> : null
                            }

                            <div className="preview" style={previewStyles}>
                                <img draggable="false" src={this.state.thumbSrc} />
                                <span className="stack-info clearfix">
                                    { this.props.stackId ?
                                        <span className="stack-order">
                                            {this.props.stackItemNumber} / {this.props.stackItemCount}
                                        </span> : null }
                                    { occurrences.length > 1 ?
                                        <span className="stack-duplicate">{occurrences.length}</span> : null }
                                </span>
                                    ...
                                    this.state.uploadProgress!==null ?
                                        <span className="upload-progress">
                                            <div className="progress cancel">
                                                <span className="meter" style={uploadMeterStyle}></span>
                                            </div>
                                        </span> : null
                                }
                                <div className="options-bar">
                                    { 
                                        !this.props.starred &&
                                        <span className="favorite-star" onClick={this.onStarClick}></span> 
                                    }
                                    {
                                    ...
                                    {
                                        occurrences.length > 0 && this.props.showPageNumbers &&
                                        <div className="image-number">{imageNum}</div>
                                    }
                                    <span title="Large Preview (spacebar)" className="details" onMouseDown={this.onFileInfoClick}></span>
                                </div>
                                {
                                    hasErrors ?
                                        <object className="img-error" type="image/svg+xml" data="/images/no-access.svg"></object> : null
                                }

                                {
                                    this.props.starred ?
                                        <span className="favorite-active" onClick={this.onStarClick}></span> : null
                                }
                            </div>

                            { this.props.showInfo ? <span className="filename">{this.props.image.filename}</span> : null }

                            {
                                (this.props.stackItemNumber === 1 && this.props.stackCollapsed) ?
                                    <span className="stack-preview">
                                        <div className="rotator">
                                            <span className="item img-container">
                                                <img src={this.state.thumbSrc} />
                                                <span className="stack-info clearfix">
                                                    <span className="stack-order">{this.props.stackItemCount}</span>
                                                </span>
                                            </span>
                                            <div className="shadow-page-1"></div>
                                            <div className="shadow-page-2"></div>
                                        </div>
                                    </span> : null
                            }
                            ...
                            {
                                showStackHandle ?
                                    <span className="stack-handle">
                                        <span className="stack-handle-arrow"></span>
                                    </span> : null
                            }
